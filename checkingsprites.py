...
def verify_image_size(image_path, expected_width, expected_height):
    if not os.path.exists(image_path):
        print(f"File missing: {image_path}, will reprocess.")
        return True  # Reprocess if file is missing
    try:
        image = Image.open(image_path)
        return image.width > expected_width or image.height > expected_height
    except UnidentifiedImageError:
        print(f"Corrupt or unreadable file detected: {image_path}, will reprocess.")
        return True  # Reprocess if the file is unreadable
...
def process_sprites(textfile, output_folder):
    ...
    for sprite_path in sprite_paths:
        ...
        # If file exists, check if it is larger than expected or unreadable
        if output_filename in processed_files:
            if not verify_image_size(output_path, frame_width, frame_height):
                print(f"Skipping {sprite_path} (Already processed correctly)")
                continue
            else:
                print(f"Redoing {sprite_path} (Incorrect size or corrupt file detected)")
